...
const FamilyTreeList = () => {
  ...
  const createFamilyTree = async (_name, _description, _file) => {
    ...
    try {
      let tx1 = await myFamilyContract.callStatic.createFamily(
        _name,
        metadata.url
      );
  ...
  const shareFamilyTree = async (_userAddress, relation, _familyId) => {
    ...
    try {
      let tx1 = await myFamilyContract.callStatic.addFamilyMembers(
        _userAddress,
        familyId.current,
        relation
      );
      let tx2 = await myFamilyContract.addFamilyMembers(
        _userAddress,
        familyId.current,
        relation
      );
  ...
  useEffect(() => {
    async function getFamilyTrees() {
      if (ownerAddress) {
        if (chainId == 0) {
          ...
          console.log(ownerAddress);
          const data = await getMintedTokenURIs(
    ...
    if (web3ProviderConnected) getFamilyTrees();
    // else
    //loginWeb3Auth()
  ...
  return (
    <section className="overflow-x-auto  w-full mt-2 px-14 ">
      <div className="border-t border-white/10 bg-black bg-opacity-70 rounded-xl pt-11">
        ...
        {isEditingEnabled && (
          <button
            ...
            className="mx-4 p-2 mb-5 inline-flex w-6/12 items-center justify-center rounded-md border-2 border-primary bg-primary px-5 text-base font-semibold text-white transition-all hover:text-indigo-500 hover:border-indigo-500"
          >
            Create Family Tree
          </button>
        )}
